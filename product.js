...
function renderLongDescription(){
    ...
    // "long description" is its own column (H) in the sheet — this does NOT
    // fall back to the short description anymore. If it's blank, that means
    // the cell itself is empty for this product; fill it in on the sheet
    // and it'll show here automatically, no code change needed.
    el.textContent = currentProduct.longDesc && currentProduct.longDesc.trim()
        ? currentProduct.longDesc
        : "No detailed description added for this product yet.";
}
...
// Cloudinary auto-generates a JPG frame for any video it hosts when you
// request the same path with a .jpg extension instead of the video's own —
// this gives us a real thumbnail/poster without any extra data in the sheet.
...
function hideAllViewers(){
    ...
    videoViewer.removeAttribute("poster");
    youtubeViewer.src = "";
}
...
function playVideo(url){
    videoViewer.onerror = null;

    const poster = videoPoster(url);

    if(poster) videoViewer.setAttribute("poster", poster);
    else videoViewer.removeAttribute("poster");

    videoViewer.src = url;
    videoViewer.load();   // makes sure the new source actually takes effect

    videoViewer.onerror = () => {
        showEmpty("This video could not be loaded — please check the file link.");
    };
}
...
function renderVideos(){
    ...
    playVideo(media.videos[0]);

    media.videos.forEach((url, i) => {
        const poster = videoPoster(url);

        const thumb = document.createElement("div");
        thumb.className = "thumb video-thumb" + (i === 0 ? " active" : "");
...
function youtubeId(url){
    ...
    // guards against placeholder/incomplete links like "www.youtube.com"
    return /^[a-zA-Z0-9_-]{10,12}$/.test(id) ? id : null;
}
...
function renderYoutube(){
    ...
    youtubeViewer.src = "https://www.youtube.com/embed/" + valid[0].id;
    ...
    valid.forEach((item, i) => {
        ...
        img.className = "thumb" + (i === 0 ? " active" : "");
        img.onclick = () => {
            youtubeViewer.src = "https://www.youtube.com/embed/" + item.id;
            document.querySelectorAll(".thumb").forEach(t => t.classList.remove("active"));
            img.classList.add("active");
        };
        thumbnailStrip.appendChild(img);
